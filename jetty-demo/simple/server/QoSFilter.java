...
{
	Logger logger = LoggerFactory.getLogger("aldsfjalsdfj");
    ...
    {       
        ...
        logger.info("maxRequest = " + _maxRequests);
        logger.info("waitMs = {}, supendMs = {}",_waitMs, _suspendMs);
    }
    ...
    throws IOException, ServletException
    {
        ...
        try
        {
            logger.info("QoSFilger doFilter 1");
...
//                else
            ...
            {
            	logger.info("QoSFilger doFilter 2");
                ...
                if (suspended.booleanValue())
                {
                    ...
                    if (request.getAttribute("javax.servlet.resumed")==Boolean.TRUE)
                    {
                        _passes.acquire();
                        accepted=true;
                        logger.info("try acquire 1");
                    }
                    else 
                    {
                        // Timeout! try 1 more time.
                        accepted = _passes.tryAcquire(_waitMs,TimeUnit.MILLISECONDS);
                        logger.info("try acquire 2");
                    }
                }
                else
                {
        ...
        finally
        {
            if (accepted)
            {
                for (int p=_queue.length;p-->0;)
                {
                    ...
                    {
                        logger.info("accepted, and resume one");
                    	continutaion.resume();
                        break;
                    }
                }
